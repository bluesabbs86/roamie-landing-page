...
serve(async (req) => {
  ...
  try {
    ...
    const { departure, destination, checkIn, checkOut, nights, adults, children, totalBudget, currencyCode, currencySymbol } = await req.json();
...
Use realistic mid-range estimates. Return numbers only, no ranges.`;
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are a travel cost estimation assistant. Return only valid JSON with numeric values. No markdown." },
